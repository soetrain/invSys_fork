...
| Presentation restart | Current candidate 25 PASS/two harness failures; two prior checks unreached. Saved preference/exact pairing assertions pass; restored Compare image is blocked by an insufficient-memory dialog. Two captures accepted; one rejected plus one diagnostic. GDI peak reaches configured 10,000 quota; 301 native XLMAIN windows observed. Empty test Excel requires assisted termination. | Localize resource growth around fresh-session pairing/rendering/capture before retrying. Earlier 27/27 remains historical. No clean closure or complete restart acceptance; see [presentation record](plan022_slice4be_guide_presentation_results.md). |
