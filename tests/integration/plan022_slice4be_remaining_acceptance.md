...
| Reusable Production palette | Current multiline candidate: captured predecessor RED 12 PASS/four expected height FAIL becomes 16/16, retaining all 11 original checks. Four reviewed sizes show eight display-only rows. Separate focused shutdown RED 11 PASS/one FAIL becomes 12/12 through the actual Close handler and isolated cleanup. Earlier broader reusable gate retains 67 Boolean observations, with only EightPaletteRows changing False to True. | Preserve scoped palette visibility; predecessor also shows eight short rows, so height RED is not clipping proof. Add a focused header test for the clipped Committed / Used heading before correction. Maximized lower-status tooltip prevents whole-form capture acceptance. The broader reusable harness's cleanup remains unproven. See [palette/fixture evidence](plan022_production_palette_results.md). |
