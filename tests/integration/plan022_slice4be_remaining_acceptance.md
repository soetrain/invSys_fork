...
| 4be.3 Comprehensive Events | Published projection, paging/filtering, contributing lines, selected detail and policy-aware reads have recorded tests. Approved Event Detail native scrolling has 44/44 focused GREEN and four reviewed captures. | Retain complete family/control coverage as 4be.1 expands. Finish broader candidate gates and complete multiline rendering. Preserve read-only authority and all prior Viewer identities. |
...
- **Event Detail approved 2026-09-24:** allow Locked=False only for
  ...
  the single-form component delta pass. Broader candidate and human acceptance,
  plus separate multiline readability, remain open.
