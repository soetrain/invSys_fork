...
| Full chain/live roles/Create Warehouse | Passive control retains 32/48/15 passes and all prior identities; settings/reports restored and hashes unchanged. Original restart Excel still crashes in combase.dll (c0000005), followed by an Office module-version-mismatch event, without external post-Quit COM reattachment or additional Quit. Excel is closed; the cleanup guard terminated no process. | Isolate restart shutdown with a bounded control. Preserve behavioral evidence and both failure events; do not repeat the broad chain unchanged or infer clean shutdown from exit code 0. |
