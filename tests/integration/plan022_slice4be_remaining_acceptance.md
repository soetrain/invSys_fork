...
| Presentation restart | Saved-workbook behavior 34/34, retaining all 27 prior identities plus five host and two exact preference-observation checks; five compiles, normal closure, restored settings, zero Application failures and unchanged hashes. Guard calibration 32/32. Add-in-only window retention reproduces in a synthetic XLAM; the earlier quota failure remains. Current three reviewed captures have a Task Manager overlay; three clean captures from an earlier completed attempt remain separate evidence. | Diagnose the 84.485-second selected-view observation and obtain unobscured current captures. Do not claim operator responsiveness or an add-in-only fix. See [resource and preservation evidence](plan022_slice4be_gui_resource_results.md). |
