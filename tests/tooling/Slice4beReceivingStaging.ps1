...
function Test-ReceivingControlRecords($Fixture,[string[]]$Before,[string]$ControlId,
    ...
    foreach ($record in $records) {
        ...
        $readable = $readable -and $record.CatalogVersion -in @(3,4,5,6,7,8,9,10) -and $record.CatalogVersion -ge $MinimumCatalog -and (Get-ActivityRead $record.RecordId).StartsWith('OK|')
    }
