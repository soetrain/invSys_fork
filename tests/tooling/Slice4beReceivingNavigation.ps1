...
function Test-ReceivingNavigationRecords($Fixture,$Before,$Case,[string]$Label) {
    ...
    foreach($record in $records) {
        # Catalog7/8 retain the version6 navigation definitions; all remain supported.
        $truth=$truth -and @($record.SourceEventRefs).Count -eq 0 -and $record.CatalogVersion -in @(6,7,8)
