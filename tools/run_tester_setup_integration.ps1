...
$harnessPath = Join-Path $fixtures "TesterSetup_Integration_Harness_$harnessStamp.xlsm"
$resultPath = Join-Path $repo "tests/integration/tester-setup-results.md"
...
try {
    ...
    $modulePaths = @(
        ...
        (Join-Path $repo "tests/integration/test_TesterSetup.bas")
    )
    ...
    $passed = [int](Run-Macro -Excel $excel -WorkbookName $harness.Name -FunctionName "test_TesterSetup.TestTesterSetup_EndToEnd")
    $context = [string](Run-Macro -Excel $excel -WorkbookName $harness.Name -FunctionName "test_TesterSetup.GetTesterSetupContextPacked")
    $rowsPacked = [string](Run-Macro -Excel $excel -WorkbookName $harness.Name -FunctionName "test_TesterSetup.GetTesterSetupEvidenceRows")
    ...
    $lines = @()
    $lines += "# Tester Setup Integration Results"
    $lines += ""
    ...
    Write-Output "TESTER_SETUP_INTEGRATION_OK"
