...
function New-NormalizedImportFile {
    ...
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("invsys-wan-wh1-" + [guid]::NewGuid().ToString("N"))
...
$harnessPath = Join-Path $fixtures "WanWh1Setup_Proof_Harness_$harnessStamp.xlsm"
...
try {
    ...
    $modulePaths = @(
        ...
        (Join-Path $repo "tests/integration/prove_wan_wh1_setup.bas"),
        (Join-Path $repo "tests/integration/TestWanWh1SetupEntry.bas")
    )
    $classPaths = @()
    ...
    $passed = [int](Run-Macro -Excel $excel -WorkbookName $harness.Name -FunctionName "TestWanWh1SetupEntry.RunWanWh1SetupProof")
    $context = [string](Run-Macro -Excel $excel -WorkbookName $harness.Name -FunctionName "TestWanWh1SetupEntry.GetWanWh1SetupContext")
    $contextMap = Parse-PackedMap -Packed $context
    $resultPath = if ($contextMap.ContainsKey("ResultPath")) { $contextMap["ResultPath"] } else { "" }

    Write-Output "WAN_WH1_SETUP_PROOF_OK"
