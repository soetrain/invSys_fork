...
Private Function CreatePolicyTable(ByVal wb As Workbook, ByVal name As String, ByVal fields As Variant, _
    ...
    Loop
    Set CreatePolicyTable = table
End Function

Private Sub SetPolicyCell(ByVal table As ListObject, ByVal row As ListRow, ByVal header As String, ByVal value As Variant)
    If VarType(value) = vbString Then row.Range.Cells(1, modTrackingPolicyModel.TableColumn(table, header)).NumberFormat = "@"
    row.Range.Cells(1, modTrackingPolicyModel.TableColumn(table, header)).Value2 = value
End Sub
