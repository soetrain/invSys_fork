...
| --- | --- | --- | --- |
| Receiving - Confirm Writes | `ReceivedTally` worksheet button, `modTS_Received.ConfirmWrites` | Partial | Core receive-post path exists and is capability-gated. Needs validation in generated real operator workbook after Tester Station cleanup. |
...
1. Implement **Delete Tester Station Generated**.
2. Fix **Verify Add-ins Published**.
3. Decide the correct generated operator workbook location for Tester Station.
4. Run **Create New Warehouse** against the NAS path and document exactly what it creates.
5. Replace or upgrade older Create/Delete/Edit User forms so they manage the real `Auth.xlsb` users/capabilities model.
6. Define the final Admin Console shape for R1.
